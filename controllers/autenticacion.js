...
function verificarToken(req, res, next) {
  ...
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
      if (err) {
          return res.status(500).send({ message: 'No se pudo autenticar el token.' });
      }
...
async function login(req, res) {
    ...
    db.query(sql, [username, password], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            const cookieOption = {
              expires: new Date(Date.now() + 2000*60), // process.env.ACCESS_COOKIE_EXPIRATION * 3600 * 24 * 1000 -- expira en 1 dia
              path: '/',
            }
...
module.exports = { login, registro, registro_licencia, verificarToken };